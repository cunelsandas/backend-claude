...
const dailyChartData = computed(() => {
    const days = props.charts.daily.map(d => d.day);
    return {
        labels: days,
        datasets: [
            {
                label: 'Total',
                data: props.charts.daily.map(d => d.total),
                borderColor: 'rgb(196,137,47)',
                backgroundColor: 'rgba(196,137,47,0.08)',
                fill: true, tension: 0.3, pointRadius: 2,
            },
            {
                label: 'Retail',
                data: props.charts.daily.map(d => d.retail),
                borderColor: 'rgb(63,111,154)',
                backgroundColor: 'rgba(63,111,154,0.06)',
                fill: true, tension: 0.3, pointRadius: 2,
            },
            {
                label: 'Wholesale',
                data: props.charts.daily.map(d => d.wholesale),
            ...
            {
                label: 'Marketplace',
                data: props.charts.daily.map(d => d.marketplace),
                borderColor: 'rgb(179,73,63)',
                backgroundColor: 'rgba(179,73,63,0.06)',
                fill: true, tension: 0.3, pointRadius: 2,
            },
        ],
    };
...
const CHANNEL_COLORS = {
    ...
    Lineat:   'rgba(6,199,85,0.7)',
    Shop:     'rgba(196,137,47,0.7)',
    Lazada:   'rgba(245,130,32,0.7)',
    Shopee:   'rgba(238,77,45,0.7)',
    Tiktok:   'rgba(0,0,0,0.6)',
    Online:   'rgba(63,111,154,0.7)',
                    ...
                    <table class="w-full text-sm tnum">
                        ...
                        <tbody>
                            <tr v-if="!saleRank.length">
                                <td colspan="6" class="px-4 py-6 text-center text-[color:var(--color-brand-400)] text-xs">No data</td>
                            </tr>
                            <tr
                                v-for="(emp, i) in saleRank"
                                :key="emp.sma_id"
            ...
            <div class="p-5">
                <template v-for="tab in commTabs" :key="tab.key">
                    <div v-if="activeCommTab === tab.key" class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <template v-if="commissions[tab.key].per_employee !== undefined">
                            <div>
